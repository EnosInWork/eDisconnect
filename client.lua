...
Citizen.CreateThread(function()
    while true do
        ...
        for k,v in pairs(cached_players) do
            if #(v.pos - pCoords) < Config.DrawDistance then
                pNear = true
                DrawText3DSecond(v.pos.x, v.pos.y, v.pos.z+0.07, Config.Translation.FirstDraw)
                DrawText3D(v.pos.x, v.pos.y, v.pos.z, "~s~"..Config.Translation.reason.." "..v.res.. "\n"..Config.Translation.time..""..v.date) 
            end
        end
